from flask import Flask, make_response, request, render_template, redirect, send_from_directory, abort
...
import chaves
import requests
from macro import cal_macro as macro
...
#RecaptchaV3
SITE_KEY = "6LdtL7olAAAAABnq0oy8nRWpuS9-F9_75NS0AfSa"
SECRET_KEY = "6LdtL7olAAAAAHksVuLP8o97JaRZHG7IIybvHlby"
...
def menu():
    ...
    # Monta a resposta.
    return render_template("menu.html", logado = logado, mensagem = "", site_key=SITE_KEY)
...
def enviar_email():
    ...
    if logado is None:
        return redirect("/")
    corpo_email = request.form["mensagem"]
    
    msg = email.message.Message()
    msg['Subject'] = "Feedbacks"
    msg['From'] = 'stunderbr@gmail.com'
    msg['To'] = 'stunderbr@gmail.com'
    password = 'dctnnufqfogtbubh'
    ...
    print('Email enviado')
    return render_template("feedbacks.html", logado = logado)
...
def calc_alimentos():
    ...
    return render_template("calc_alimentos.html", logado = logado)

@app.route('/resultadoAlimentos', methods=['GET','POST'])
def mostrar_macros():
    ...
    dados = request.form['nomeFruta']
    resp_api = macro(dados)
   
    resposta = make_response(render_template("resultAlimentos.html", logado=logado, dados = resp_api))
    return resposta
### Cadastro de séries. ###
...
def sobreNos():    
    ...
    # Monta a resposta.
    return render_template('sobreNos.html', logado = logado)
